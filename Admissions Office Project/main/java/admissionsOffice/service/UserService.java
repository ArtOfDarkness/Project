...
import admissionsOffice.dao.UserRepository;
...
public class UserService implements UserDetailsService {
	...
	public void sendActivationCode(User user) {
		...
		if (!StringUtils.isEmpty(user.getEmail())) {
            String message = String.format(
            		"Доброго дня, %s %s! \n\n" +
            				"Вітаємо у додатку \"Приймальна комісія\".\n" +
            				"Для продовження реєстрації і активації свого аккаунта перейдіть по ссилці:\n" +
            ...
            mailSender.send(user.getEmail(), "Код активації аккаунта", message);
        }
	}
    ...
    public Map<String, String> getUserErrors(Map<String, String> form) {
    	...
    	if (StringUtils.isEmpty(form.get("firstName"))) {
    		errors.put("firstNameError", "Імя користувача не може бути пустим!");
    	}
    ...
    public Map<String, String> getProfileErrors(User user, String firstName, String lastName, String email,
    	...
    	if (StringUtils.isEmpty(email)) {
    		errors.put("emailError", "Email користувача не може бути пустим!");
    	}
    	...
    	if (confirmPassword.length() < 6) {
    		errors.put("confirmPasswordError", "Пароль користувача повинен не менше 6 символів!");
    	}
    	
    	if (password != "" && confirmPassword != "" && !password.equals(confirmPassword)) {
    		errors.put("confirmPasswordError", "Введені паролі не співпадають!");
    	}
